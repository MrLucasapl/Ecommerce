...
import (
	...
	"github.com/MrLucasapl/Ecommerce.git/internal/entity"
)
...
func (pd *ProductDB) GetProductByCategoryID(categoryID string) ([]*entity.Product, error) {
	rows, err := pd.db.Query("SELECT id, , description, price, category_id, image_url FROM products where category_id = ?", categoryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
